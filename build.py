...
hbb_name = 'hanadesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: hanadesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: Hanasis Platform <platform@hanasis.com>
Homepage: https://desk.hanasis.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/hanadesk')
    system2('mkdir -p tmpdeb/etc/hanadesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/hanadesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/hanadesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/hanadesk/')
    system2(
        'cp ../res/hanadesk.service tmpdeb/usr/share/hanadesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/hanadesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/hanadesk.svg')
    system2(
        'cp ../res/hanadesk.desktop tmpdeb/usr/share/applications/hanadesk.desktop')
    system2(
        'cp ../res/hanadesk-link.desktop tmpdeb/usr/share/applications/hanadesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/hanadesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/hanadesk/')
    system2(
        'cp ../res/pam.d/hanadesk.debian tmpdeb/etc/pam.d/hanadesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/hanadesk/files/polkit && chmod a+x tmpdeb/usr/share/hanadesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb hanadesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('hanadesk.deb', '../hanadesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/hanadesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/hanadesk/')
    system2(
        'cp ../res/hanadesk.service tmpdeb/usr/share/hanadesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/hanadesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/libhanadesk.dylib target/release/libhanadesk.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/HanaDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"HanaDesk Community Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon HanaDesk.app 200 190 --hide-extension HanaDesk.app hanadesk.dmg ./build/macos/Build/Products/Release/HanaDesk.app")
    os.rename("hanadesk.dmg", f"../hanadesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libhanadesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack, skip_sign=False, admin=False):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/hanadesk.exe')
    os.chdir('../..')
    if os.path.exists('./hanadesk_portable.exe'):
        os.replace('./target/release/hanadesk-portable-packer.exe',
                   './hanadesk_portable.exe')
    else:
        os.rename('./target/release/hanadesk-portable-packer.exe',
                  './hanadesk_portable.exe')
    print(
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/hanadesk.exe target/release/HanaDesk.exe')
        if not skip_sign:
            sign_file('target\\release\\HanaDesk.exe')
        else:
            print('Signing skipped (--skip-sign)')
        system2(
            f'cp -rf target/release/HanaDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/hanadesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/hanadesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/hanadesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv hanadesk-%s-0-x86_64.pkg.tar.zst hanadesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./hanadesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/hanadesk-%s-0.x86_64.rpm ./hanadesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall hanadesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/hanadesk-%s-0.x86_64.rpm ./hanadesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall hanadesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/hanadesk*.deb ./flutter/hanadesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/HanaDesk.app/Contents/MacOS/hanadesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/HanaDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/hanadesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/HanaDesk.app/Contents/MacOS/hanadesk
    #rcodesign sign --p12-file ~/.p12/hanadesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/HanaDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/hanadesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/HanaDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/HanaDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/HanaDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "HanaDesk %s.dmg" "target/release/bundle/osx/HanaDesk.app"' % version)
                os.rename('HanaDesk %s.dmg' %
                          version, 'hanadesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/hanadesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./hanadesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./hanadesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple hanadesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/HanaDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/hanadesk*.deb ./hanadesk.deb')
                system2('dpkg-deb -R hanadesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/hanadesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/hanadesk-link.desktop tmpdeb/usr/share/applications/hanadesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/hanadesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/hanadesk/')
                os.system('mkdir -p tmpdeb/etc/X11/hanadesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/hanadesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/hanadesk.debian tmpdeb/etc/pam.d/hanadesk')
                system2('strip tmpdeb/usr/bin/hanadesk')
                system2('mkdir -p tmpdeb/usr/share/hanadesk')
                system2('mv tmpdeb/usr/bin/hanadesk tmpdeb/usr/share/hanadesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/hanadesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb hanadesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('hanadesk.deb', 'hanadesk-%s.deb' % version)
