...
#define HANADESK_LIB_PATH "libhanadesk.so"
typedef bool (*HanaDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_hanadesk_core_main() {
   void* libhanadesk = dlopen(HANADESK_LIB_PATH, RTLD_LAZY);
   if (!libhanadesk) {
      fprintf(stderr,"Failed to load \"libhanadesk.so\"\n");
      char* error;
   ...
   auto core_main = (HanaDeskCoreMain) dlsym(libhanadesk,"hanadesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"hanadesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
