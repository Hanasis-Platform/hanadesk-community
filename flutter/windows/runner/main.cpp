...
typedef char** (*FUNC_HANADESK_CORE_MAIN)(int*);
typedef void (*FUNC_HANADESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_HANADESK_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libhanadesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libhanadesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_HANADESK_CORE_MAIN hanadesk_core_main =
      (FUNC_HANADESK_CORE_MAIN)GetProcAddress(hInstance, "hanadesk_core_main_args");
  if (!hanadesk_core_main)
  {
    std::cout << "Failed to get hanadesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_HANADESK_FREE_ARGS free_c_args =
      (FUNC_HANADESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = hanadesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
  ...
  std::wstring app_name = L"HanaDesk Community";
  FUNC_HANADESK_GET_APP_NAME get_hanadesk_app_name = (FUNC_HANADESK_GET_APP_NAME)GetProcAddress(hInstance, "get_hanadesk_app_name");
  if (get_hanadesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_hanadesk_app_name(app_name_buffer, 512) == 0) {
