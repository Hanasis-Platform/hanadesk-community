...
fn get_update_temp_dir() -> PathBuf {
    ...
    Path::new("/tmp").join(format!(".hanaDeskupdate-{}", euid))
}
...
// remove just one app from all the permissions: tccutil reset All com.hanasis.hanadesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.hanasis.hanadesk", &bundleid);
    }
    s = s.replace("hanadesk", &crate::get_app_name().to_lowercase());
    s = s.replace("HanaDesk", &crate::get_app_name());
    s
}
